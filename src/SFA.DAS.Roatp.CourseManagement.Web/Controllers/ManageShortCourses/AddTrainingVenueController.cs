...
public class AddTrainingVenueController(ISessionService _sessionService, ILogger<AddTrainingVenueController> _logger) : ControllerBase
{
    ...
    public Task<IActionResult> LookupAddress(CourseType courseType, [FromRoute] string larsCode)
    {
        ...
        if (isAddJourney)
        {
            ...
            if (sessionModel.LocationsAvailable)
            {
                ...
                return Task.FromResult<IActionResult>(RedirectToRoute(RouteNames.SelectShortCourseTrainingVenue, new { ukprn = Ukprn, courseType }));
            }
        }
    ...
    public Task<IActionResult> LookupAddress([FromForm] AddTrainingVenueSubmitModel submitModel, CourseType courseType, [FromRoute] string larsCode)
    {
        var isAddJourney = IsAddJourney(larsCode);
        ...
        return Task.FromResult<IActionResult>(RedirectToRoute(RouteNames.GetConfirmAddTrainingVenue, new { ukprn = Ukprn, courseType }));
    }
